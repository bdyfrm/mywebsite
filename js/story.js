document.addEventListener("DOMContentLoaded", () => {
  ...
  const heartBack = document.getElementById("heartBack");
  ...
  let currentIndex = 0;
  const slideDuration = 9000; // 9 seconds per slide
  let elapsedTime = 0;
  let isPaused = false;
  let lastFrameTime = 0;
  ...
  function isVideoItem(item) {
    ...
    const source = item.media || item.photo || "";
  ...
  function renderStories() {
    ...
    timelineData.forEach((item, index) => {
      ...
      const mediaSource = item.media || item.photo || "";
      const mediaType = isVideoItem(item) ? "video" : "image";
      const hasMedia = !!mediaSource;
      const bgClass = hasMedia && mediaType === "image" ? "" : " no-photo";
      const mediaMarkup = !hasMedia
        ? ""
        : mediaType === "video"
          ? `<video class="slide-photo slide-video" muted autoplay loop playsinline webkit-playsinline preload="auto">
              <source src="${mediaSource}" type="video/mp4" />
            </video>`
          : `<img src="${mediaSource}" alt="${item.title}" class="slide-photo" />`;

      slide.innerHTML = `
        <div class="slide-bg${bgClass}" style="--photo-bg: ${photoBackground(mediaType === "image" ? mediaSource : "", index)}"></div>
        ${mediaMarkup}
  ...
  function getActiveVideo() {
    ...
    return slides[currentIndex]?.querySelector("video.slide-video") || null;
  }
  ...
  if (heartBack) heartBack.addEventListener("click", prevSlide);
  if (heartNext) heartNext.addEventListener("click", nextSlide);
